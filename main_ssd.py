...
"""
import os
from evaluate_ssd import evaluate_ssd_base
...
def roll_out(networks, env, args, init_set, epi_num, explore_params, paths, is_draw=False, is_train=True):
    ...
    agent_types = list(env.agents_types.values())  # env.agents_types = {agent_id: int}
    ...
    for i in range(epi_length):
        ...
        if args.mode_ac:
            ...
            if is_train and rand_prob < epsilon:
                act = {agent_id: env.action_space[agent_types[idx]].n for idx, agent_id in enumerate(agent_ids)}
        ...
        # Step.
        obs, act, rew, cost, m_act, n_obs, fea = env.step(act, args.lv_incentive)

        #build utility for training purposes
        utility = {key: rew[key] - cost[key] for key in rew}
        # Add one-transition sample to samples if is_train=True.
        if is_train:
            samples[i] = (obs, act, utility, m_act, n_obs, fea, beta)

        # Update collective_reward and collective_feature for each type.
        for idx, agent_id in enumerate(agent_ids):
            agent_type = agent_types[idx]
            collective_reward[agent_type] += (rew[agent_id] - cost[agent_id])
...
single_alpha = args.lv_incentive
alphas_env = [single_alpha]
paths_pol_dir_dict = {
    #single_alpha: saved_path+'/000004999.tar',
    single_alpha: saved_path+'/000034999.tar',  # Run files that have .tar extension in the folder.
    # single_alpha: '../results_ssd/test_220914_1434/saved/000014999.tar',  # Run one file.
}
num_tests = 100
is_draw = False

evaluate_ssd_base.evaluate_ssd(alphas_env, paths_pol_dir_dict, num_tests, is_draw, single_alpha)
