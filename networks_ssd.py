...
class Networks(object):
    ...
    def __init__(self, env, args):
        ...
        self.num_agents: list = env.num_agents
        self.agents_types = env.agents_types
    ...
    def get_observation_size(self, env: Union[CleanupEnvReward, CleanupEnvMultiType, HarvestEnvReward]) -> int:
    ...
    def get_action_size(self, env: Union[CleanupEnvReward, CleanupEnvMultiType, HarvestEnvReward]) -> list:
        action_size = []
    ...
    def get_actions(self, obs: dict, prev_m_act: list, beta: float, is_target: bool = True):
        ...
        agent_types = list(self.agents_types.values())  # env.agents_types = {agent_id: int}
        ...
        with torch.no_grad():
            ...
            for agent_type in range(self.num_types):
                ...
                if self.args.mode_ac:  # based on actor.
                    if is_target:
                        action_probs = self.actor_target[agent_type](obs_input)
                    else:
                        action_probs = self.actor[agent_type](obs_input)
                else:
    ...
    def preprocess(self, samples: list):
        ...
        for agent_type in range(self.num_types):
            obs_t[agent_type] = np.array(obs_t[agent_type])
            act_t[agent_type] = np.array(act_t[agent_type])
    ...
    def to_tensors(self, obs_t=None, act_t=None, rew_t=None, m_act_t=None, n_obs_t=None, fea_t=None, beta_t=None):
        ...
        with torch.no_grad():
            ...
            if act_t is not None:
                act_t_tensor = []
                for agent_type in range(self.num_types):
                    act_tensor = torch.tensor(act_t[agent_type], dtype=torch.int64)  # Shape should be (N,) .to(self.device) GPU
            ...
            if n_obs_t is not None:
                ...
                for agent_type in range(self.num_types):
                    if self.args.mode_one_hot_obs:
                        # F.one_hot takes tensor with index values of shape (*) and returns a tensor of shape (*, num_classes)
                        n_obs_tensor = torch.tensor(n_obs_t[agent_type], dtype=torch.int64) # .to(self.device) GPU
            ...
            if beta_t is not None:
                beta_t_tensor = []
                for agent_type in range(self.num_types):
                    beta_tensor = torch.tensor(beta_t[agent_type], dtype=torch.float)   # Shape should be (N,) .to(self.device) GPU
