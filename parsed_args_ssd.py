...
def str_to_list(s):
    # Expecting input in the format "[item1, item2, ...]"
    try:
        return eval(s)
    except:
        raise argparse.ArgumentTypeError("List must be in the format '[item1, item2, ...]'")
...
def add_default_args(parser):
    ...
    parser.add_argument("--env", type=str, default="cleanup_modified", help="Name of the environment to use.")
    parser.add_argument("--num_types", type=int, default=2, help="Number of agents' types.")
    parser.add_argument("--num_agents", type=str_to_list, default="[5, 5]", help="Number of agents for each type.")
    ...
    parser.add_argument("--num_episodes", type=int, default=35000, help="Number of episodes.")
...
""" Setting for the description. """
args.description = 'linear_contract_design_experiment_cleanup'
#args.setting_name = 'test'+utils_all.get_current_time_tag()+'alpha=92_ncleaners=5_num_t=200_num_ep=35000_save_freq=200_gamma=09'
#args.setting_name = f'test{utils_all.get_current_time_tag()}_num_agents={args.num_agents[0],args.num_agents[1]}_lv_incentive={args.lv_incentive}'
args.setting_name = f'exp{utils_all.get_current_time_tag()}_na_{args.num_agents[0]}_{args.num_agents[1]}_tax_{int(args.lv_incentive * 100)}'
""" Setting for the environment. 
...
args.num_types = 2
#args.num_agents = [5, 5] #1 entry = cleaner 2 entry = harvester
args.rew_clean = 0.00
args.rew_harvest = 1.00
...
""" Setting for the networks. """
args.mode_ac = False  # True if actor-critic/psi.
...
""" Setting for the experiment. """
#args.num_episodes = 35000 #10000 35000
args.episode_length = 200 #1000
args.epsilon = 0.9
