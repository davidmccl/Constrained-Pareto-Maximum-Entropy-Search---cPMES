...
def roll_out(networks, env, args, init_set, epi_num, explore_params, paths, is_draw=False, is_train=True):
    ...
    collective_reward = [0 for _ in range(len(agent_ids))]
    collective_cost = [0 for _ in range(len(agent_ids))]
    ...
    for i in range(epi_length):
        ...
        for idx, agent_id in enumerate(agent_ids):
            ...
            collective_reward[idx] += rew[agent_id]
            collective_cost[idx] += cost[agent_id]
    ...
    return samples, init_set, collective_reward, collective_cost, collective_feature
...
if __name__ == "__main__":
    ...
    # Metrics
    collective_rewards, collective_rewards_test = [np.zeros([sum(args.num_agents), args.num_episodes]) for _ in range(2)]
    ...
    for i in range(args.num_episodes):
        ...
        buffer += samples
        for agent_type in range(len(collective_reward)):
